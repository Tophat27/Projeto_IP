...
def iniciar_jogo():
    ...
    inventory = Inventory()
    # Cenarios
    caminhos_fundos = ["images/entrada_ufpe.png", "images/bib_central.png", "images/ru.png", "images/CIn.png", "images/Gesonel.webp"]
    cenarios = []
    ...
    while True:
        ...
        # Draw player and enemyq
        tela.blit(player.image, player.rect.topleft)
        if enemy:
            tela.blit(enemy.image, enemy.rect.topleft)
        # Draw collectibles
        bota_rect, guarda_chuva_rect, bota_visivel, guarda_chuva_visivel, cracha_visivel, flag_cracha = coletaveis(indice_cenario, ultimo_cenario, player.rect.x, player.rect.y, player.image, bota_visivel, guarda_chuva_visivel, inventory, cracha_visivel)
        # Draw inventory
        inventory.draw(tela)
