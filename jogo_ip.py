...
class Menu:
    """Classe para gerenciar o menu principal"""
    ...
    def mostrar_creditos(self):
        ...
        while mostrando:
            ...
            self.config.tela.fill((0, 0, 0))
            texto = self.config.fonte_grande.render("Créditos", True, self.config.branco)
            texto_nome = self.config.fonte_media.render("Desenvolvido por Pedro (phhs)", True, self.config.branco)
            texto_esc = self.config.fonte_media.render("Pressione ESC para voltar", True, self.config.branco)
            
            self.config.tela.blit(texto, (self.config.largura // 2 - 150, 100))
            self.config.tela.blit(texto_nome, (self.config.largura // 2 - 300, 300))
            self.config.tela.blit(texto_esc, (self.config.largura // 2 - 250, 500))
    ...
    def executar(self):
        ...
        while True:
            self.config.tela.blit(self.recursos.imagem_tela_inicio, (0, 0))
            
            self.interface.desenhar_botao(self.config.tela, self.recursos.jogar_buttom, 
                                        self.config.largura // 2 - 150, self.config.altura // 2 - 100, 
                                        lambda: Jogo().executar())
            self.interface.desenhar_botao(self.config.tela, self.recursos.creditos_buttom, 
                                        self.config.largura // 2 - 150, self.config.altura // 2 + 0, 
                                        self.mostrar_creditos)
            self.interface.desenhar_botao(self.config.tela, self.recursos.sair_buttom, 
                                        self.config.largura // 2 - 150, self.config.altura // 2 + 100, 
                                        pygame.quit)
...
if __name__ == "__main__":
    ...
    recursos = CarregadorRecursos(config)
    menu = Menu(config, recursos)
    menu.executar()